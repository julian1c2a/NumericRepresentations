...
template<std::size_t B,std::size_t L>
constexpr inline
std::uint64_t convert_to_int(const register_of_digits_t<B,L> & arg) noexcept {
	std::uint64_t cB = B;
...
test_result_t test_dig_suma_dig_con_asignacion() {
	...
	for(std::uint64_t ix{0} ; ix < B2L ; ++ix) {
		...
		for(std::uint64_t iy{0} ; iy < B2L ; ++iy) {
			...
			if (!bien) {
				//d_t rd_w{rd_x};
				//m_sum_digs_carryin_0(rd_w,rd_y);
				++errores;
			} else {
				++correctos;
			}
...
void display_constr_SINT_dig_t(){
	...
	std::int64_t k{-1000}; std::int64_t kold{0};
	for (std::int64_t i{-1030} ; i<1030 ; ++i) {
		base_B_dig_t unidades(i);
		if ((k%static_cast<std::int64_t>(B))!=static_cast<std::int64_t>(B-1)) {
...
void display_operator_assign_UINT_dig_t()									{
	using base_B_dig_t = digit_t<B>;
	std::uint64_t k{0}; std::uint64_t kold{0};
