...
public:
	...
	/// PARA B=3 2*2																			 1  COMIENZAN POR 2 B-2
	/// PARA B=4 2*2 2*3 3*4															 3  COMIENZAN POR 2 B-2, 3 B-3
	/// PARA B=5 2*2 2*3 2*4 3*3 3*4 4*4									 6  COMIENZAN POR 2 B-2, 3 B-3, 4 B-4
	/// PARA B=6 2*2 2*3 2*4 2*5 3*3 3*4 3*5 4*4 4*5 5*5	10  COMIENZAN POR 2 B-2, 3 B-3, 4 B-4, 5 B-5
	/// PARA B=r 2*2 2*(2+1) ... 2*(r-1)							r-1  r  COMIENZAN POR 2 B-2, 3 B-3, 4 B-4, 5 B-1, ... , r-1 B-r+1
	///					 		 3*3		 ... 3*(r-1)							r-2
	///											 ...
	///														(r-1)*(r-1)					r-(r-1)
	///																								_______
	...
	///																								------------------
	///																								SIZE = (B^2 - 3B + 2)/2
	///																								2+3+4+...+B-1 = 1+2+3+4+...+B-1 - 1 =
	///                                                (B-1)B/2 - 2(B-1)/2 = (B^2 - B - 2B +2)/2 =
	///                                                =(B^2-3B+2)/2
	///
	/// FALTARIA HACER SOLO PARA BASES MAYORES QUE 2
	/// CODE NEW USING TYPES :
	/// using pair_map = std::pair<uintspair,uintspair>;
	/// CODE MULTTABLE:
	///		static consteval std::array<pair_map,(B^2 - 3B + 2)/2> multtable{make_multtables()};
	/// CODE FOR MAKE_MULTTABLES():
	///		static consteval
	///		std::array<pair_map,(B^2 - 3B + 2)/2> make_multtables() noexcept {
	///			std::array<pair_map,(B^2 - 3B + 2)/2>	ret;
	///			/// ESTO QUE SIGUE HABRIA QUE HACERLO POR UNROOLING LOOP EN COMPILE TIME
	///			/// YA QUE IDX E IDY NO SON CONSTANTES EN TIEMPO DE COMPILACION
	///			///	LO MEJOR ES HACERLO EN UNA CLASE TEMPLATE POR RECURSION
	///			for(size_t idx{2} ; idx < B ; ++idx) {
	///				for(size_t idy{idx} ; idy < B ; ++idy) {
	///					make_pair(make_array(idx,idy),mult<idx,idy>());
	///				}
	///			}
	///		}
...
public:
	...
	inline constexpr explicit
	operator SIG_UINT_T() const
	noexcept { return static_cast<SIG_UINT_T>(m_d); }

	inline constexpr explicit
	operator SIG_SINT_T() const
	noexcept { return static_cast<SIG_SINT_T>(m_d); }
...
public:
	...
	template<type_traits::integral_c Int_t>
	constexpr inline dig_t(Int_t arg) noexcept :
		m_d(normaliza<Int_t>(arg))
	{}
	...
	/// COPIA 		: COPIA EL ARGUMENTO LITERALMENTE PARA PASARLO: EL ORIGINAL INTACTO
	/// REFERENCIA: COPIA LA DIRECCION DEL ARGUMENTO
	///             (SI ES CONST EL COMPILADOR NO TE DEJA COMPILAR SI LO CAMBIAS)
	/// MOVIMIENTO: SI EL ARGUMETO NO SE VA A UTILIZAR MAS SE LE PASA LA PROPIEDAD A LA FUNCION
	...
	template<type_traits::integral_c Int_t>
	constexpr inline
	const dig_t & operator = (const Int_t & a)
	noexcept {
		if constexpr (std::is_same_v<Int_t,UINT_T>) {
			if (&a != &m_d) {
	...
	dig_t sum_carry(dig_t arg_1,dig_t arg_2)
	noexcept {
	...
	constexpr inline
	dig_t operator & (const dig_t & arg) const
	noexcept
	{
	...
	constexpr inline
	const dig_t & operator ^= (UIntType exp)
	noexcept
	{
		dig_t& cthis{*this};
		if (exp == 0) {
	...
	template<type_traits::unsigned_integral_c UIntType>
	constexpr inline
	dig_t operator ^ (UIntType exp) const
	noexcept
	{
		dig_t cpthis{*this};
		cpthis ^= exp;
	...
	/// METODO PROPIO DE C++20
 	constexpr inline
 	std::strong_ordering operator <=> (dig_t rhs) const
 	noexcept {
	...
	template<type_traits::integral_c Int_t>
	constexpr inline
	bool operator == (Int_t rhs)
	noexcept {
		const dig_t& lhs{*this};
		return
			(
				(lhs.m_d == normaliza<Int_t>(rhs))		?
					true 					:
					false
			);
	}
	...
	template<type_traits::integral_c Int_t>
	constexpr inline
	std::weak_ordering operator <=> (Int_t rhs) const
	noexcept {
		const dig_t& lhs{*this};
	...
	constexpr inline
	dig_t operator + (dig_t arg) const
	noexcept {
		dig_t ret(*this);
		ret += arg;
		return ret;
...
std::ostream & operator << (std::ostream & os,dig_t<UINT_T,Base> arg) 	{
	os  << "d#"
		<< static_cast<std::int64_t>(arg()%Base)
		<< "#B"
