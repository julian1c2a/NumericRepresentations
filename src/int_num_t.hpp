#ifndef INT_NUM_T__HPP__INCLUDED
#define INT_NUM_T__HPP__INCLUDED
...
public:
	...
	inline void push_back(dig_t<B> arg) {
		this->num_basic_t<B>::push_back(arg);
	}
private:
...
private:
    ...
    public:
        ...
        num_int(sign_e s,const std::vector<dig> & arg,forma_t argforma=noraw) {
			...
			if (argforma==noraw) cthis.reduce();
        }
        ...
        num_int(Int_T a) {
			...
			uint indice = 0;
            if (a>=0) {
				for(int i=0;i<longitud;i++) cthis[i]=dig(0);
                signo=vplus;
                ullint rem=0,coc=a;
                while (coc >= B) {
                    rem = coc%B;
				...
				for(int i=0;i<longitud;i++) cthis[i]=dig(B-1);
                signo=vminus;
...
size_t unos_a_la_drcha () const {
	...
	if (this->operator[](finl)==0) {
		for ( int i=finl ; (i >=0) and (!Exit) ; --i) {
			if (this->operator[](i)==dig_t<B>::max) ++sz;
			else Exit=true;
		}
	}
	return sz;
...
usint digs_fracc_no_significativos () const {
	...
	return this->ceros_a_la_drcha();
        ...
        bool operator != (const num_int & arg) const {
            ...
            const slint argnsz = arg.digs_no_significativos();
            const slint argvsz = arg.size()-argnsz;
            if ((thisvsz > argvsz)||(thisvsz < argvsz)||(signo!=arg.signo)) return true;
            else {
                for (slint k=(thisvsz-1);k>0;--k)
                    if (arg[k]!=cthis[k])
                        return true;
        ...
        num_int operator + (const num_int & arg) const {
            ...
            for ( ; k<szmin ; ++k) {
                ...
                tempt += aux;
                ret[k]=tempt.g_uds();
                ret.aux=spardigs(tempt.g_sign(),
                                 ((tempt.g_sign()==vplus)?(0):(B-1)),
                                 tempt.g_decs()
                                );
            }
            for ( ; k<szmax ; ++k) {
