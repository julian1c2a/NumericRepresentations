...
inline nat_num_t& erase ( size_t pos, size_t npos ){
	...
	cthis.base_num_t::erase(pos,npos_2);
	return (*this);
}
...
inline void	div_gen_step(const nat_num_t & dvsor, nat_num_t & rem, nat_num_t & coc, usint & ndig) const {
	const nat_num_t & cthis = (*this);
...
private:
inline void div_uno( nat_num_t & rem , nat_num_t & dvndo, nat_num_t & dvsor, const usint szndo, const usint szsor) const {
	const nat_num_t & cthis = (*this);
	rem = dvndo;
	if (dvsor.es_cero()) { // caso de division por 0
		return;
	}
	else if ((dvndo.es_cero())or(szndo < szsor)) { // caso de 0 dividido por !=0 ó caso de cociente 0
		rem = dvndo;
