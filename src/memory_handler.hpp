...
public:
	void operator()(aT** var_list_list) {
			...
			static_cast<T<std::numeric_limits<uchint>::max()>*>(contruye_lista_de_variables(var_list_list[std::numeric_limits<uchint>::max()]));
			cp_new<aT,T,num_vars,std::numeric_limits<uchint>::max()-1>(var_list_list);
	}
};
...
struct cp_new<0u> {
	void operator()(aT** var_list_list) {
		var_list_list[0u]=nullptr;
	}
};

template<typename aT , template<uchint> class T , uchint B>
struct cp_delete {
private:
	static void destruye_lista_de_variables(aT* var_list) {
		delete [] matd[B];
	}
public:
	void operator()(aT** var_list_list) {
...
public:
	void operator()(aT** var_list_list) {
			destruye_lista_de_variables(var_list_list[std::numeric_limits<uchint>::max()]);
...
template<>
void destruye_matriz_de_variables<INT,INT>(INT** matd)
{
	delete *matd;
	delete [] matd;
}
...
template<>
void destruye_matriz_de_variables<UINT,UINT>(UINT** matd)
{
	delete *matd;
	delete [] matd;
}
...
private:
	...
	// Mira si un nombre de variable esta disponible y si esta libre un lugar concreto de
	// "memoria". Arregla el mapa de memoria. Si ha tenido exito y se puede insertar la
	// variable devuelve true, sino false.
    bool insert( string nombre , uchint base , uchint var_base )
    {
		...
		for( ; it != itend ; ++it) {
			if (((*it).second.second == var_base)or((*it).first==nombre)) return false;
		}
		if (mapmem[base][var_base]==false) {
			mapvd.insert(
				std::make_pair<std::string,par_dir_t>
				(
					nombre ,
					std::make_pair<uchint,uchint>(base,var_base)
				)
			);
...
public:
	...
	~func_refvar_t()
	{
		destruye_matriz_de_variables<T,T>(matd);
	}
	...
	bool escribe_var( string nombre , uchint base , uchint var_base) {
		...
		if (cuentavars[base] <= 256) {
			...
			while (! se_puede_insertar) {
				...
				if (!se_puede_insertar) var_base++;
			}
			if (se_puede_insertar) (cuentavars[base])++;
		}
...
public:
	...
	bool operator()(string nombre,string lhs)
	{
		...
		if (se_puede_hacer) (reinterpret_cast< T<B>* >(matd[B]))[mapvd[nombre].second]=temp;
...
private:
	...
	// Mira si un nombre de variable está disponible y si está libre un lugar concreto de
	// "memoria". Arregla el mapa de memoria. Si ha tenido exito y se puede insertar la
	// variable devuelve true, sino false.
    bool insert( string nombre , uchint base , uchint var_base )
    {
...
public:
	...
	~func_refvar_t()
	{
		destruye_matriz_de_variables<INT**,INT*,INT>(matd);
	}
...
public:
	...
	INT get(string nombre)
	{
		return (*matd[B])[mapvd[nombre].second];
	}
	...
	bool operator()(string nombre,string lhs)
	{
		INT temp(lhs);
		bool se_puede_hacer = este_nombre_ya_ha_sido_declarado(nombre);
		if (se_puede_hacer) (*matd)[mapvd[nombre].second]=temp;
		return se_puede_hacer;
	}
};
